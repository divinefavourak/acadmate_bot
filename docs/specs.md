...
| Math rendering | Quiz output and `/ask`/`/summarize` are math-aware: inline math is prettified to Unicode (`x^2`→`x²`, `\pi`→`π`), and display blocks (`$$…$$`, `\[…\]`) are rendered to an image via CodeCogs (falls back to monospace if rendering fails). |
